...
def filter_my_words(input_words, n):
    try:
        ...
        if not isinstance(input_words, str):
            print(f"TypeError (filter_my_words): input input_words must be a string not a {type(input_words)}.")
        elif not isinstance(n, int):
            print(f"TypeError (filter_my_words): input n must be a string not a {type(n)}.")
